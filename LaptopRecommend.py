...
def get_contentbased_laptops(laptopName, df):
    ...
    laptop_index=get_index_from_name(laptopName, df)
...
def ShowResults(top, list):
    ...
    for element in list:
        if i <= top:
            ...
            print(i, ': ', element[0], '-', element[1], '-', get_name_from_index(element[0], dfProducts))
            i+=1

##### MAIN #####
# 1. get data
dfProducts = data.getDataProducts()
dfReviews = data.getDataReviews()
inputLaptopID = 1
inputUserID = 3
